...
% FT this is a default calculation in the Gagge model, do we need to change it?
...
% FT the core temperature, in the Gagge's model, affects: the rate of energy transport between core and skin, the vasodilation/vasocontriction signal, sweathing, skin blood flow, etc.
...
% note OJ - I would say that this is not necessarily correct. Tsk will modify dry heat loss (a little bit) but at the point of compensability tsk will be max, so a simplified model can just assume max Tsk with full vasodilation (35.5C). Absolute Tcore does not really drive sweat rate, Ereq does: see Ravanelli & Jay (2020) J Physiol (London).
% FT In Gagge's model t-cr influences sweat rate
...
% FT - the model calcualtes the convective heat transfer coefficient using the following equations
...
% FT w_crit is calculated using the following equations
%    if clo <= 0:
