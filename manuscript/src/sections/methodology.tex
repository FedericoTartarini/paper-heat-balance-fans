...
One of the main differences between \mycite{GaggeSET} model and the one used by \mycite{Jay2015} is that \mycite{Jay2015} does not account for heat stored in the core or the skin compartment; hence, they assume the values of \ac{t-sk} and \ac{t-cr} to be constant.
...
% todo Ollie Jay: I think it is fair to say that a) this model assumes a steady-state, and b) the assumed Tsk values were set to represent worst-case scenario from the literature.

% todo Ollie Jay: If anything this will underestimate net heat loss from the body to surrounding environment, which is not ideal from a public health perspective. There is quite a lot of push-back on fan use due to concerns that they aggravate heat stress, so setting conservative parameters (i.e. in favour of the current viewpoint that fan worsen heat stress) is probably advisable. The other role of changing Tsk is that it serves as a forcing function for sweating, but as we previously discussed heating of the skin seems to quite dramatically alter the relationship between core temperature and sudomotor output, so that it is possible to sweat more with a fan (which elicits a higher skin temperature) despite a lower core temperature

The amount of sensible and latent heat gain or loss from the human body to its environment can be expressed as a function of environmental and personal factors~\cite{ASHRA2017}.
...
% todo Ollie Jay: I think we should acknowledge here that this does not represent the most conservative approach in terms of dry heat gain with fan use
...
Although Equation~\ref{eq:latent-skin} is expressed as a function of \ac{w}, the human body does not regulate \ac{w} directly but, rather, it regulates the sweat rate.

% todo Ollie Jay: Emerging evidence since the time of Gagge suggests otherwise. That is given evaporative rate is the goal, not sweat rate per se
...
The \ac{phs} model as described in the ISO 7933 Standard does not predict the physiological response of individual subjects and aims to evaluate working conditions for a standard subject in good health and fit for the work~\cite{iso7933}.
...
The \ac{phs} model has not been validated for metabolic rates lower than 1.7 met (100 W/m\textsuperscript{2}), \ac{t-db} higher than 50~$^{\circ}$C and water vapour partial pressure ($p_{a}$) higher than 4~kPa (i.e., humidity ratio of \var{hr_4_kpa}~kg$_{water}$/kg$_{dry air}$).
...
However, conditions indoors may be less severe than outdoors since the thermal mass of the building may dump and shift peaks in outdoor temperature.
