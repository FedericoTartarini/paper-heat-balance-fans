...
% todo EA This is only true for how Std 55 applies the model. Gagge did not use those definitions, and it's overly restrictive to put them into this text.    I would delete this; just say for the model to work, the air flow needs to affect a significant part of the body.  The best paper that looked into local airflow and the derivation of the SET model is in red:

% todo cite this paper: https://www.sciencedirect.com/science/article/pii/S0360132314000298
...
Despite the fact that Equation~\ref{eq:latent-skin} is expressed as a function of \ac{w}, the human body does not regulates \ac{w} directly but, rather, it regulates the sweat rate~\cite{ASHRA2017}.
Skin wettedness varies as a function of the activity of the sweat glands and the environmental conditions~\cite{ASHRA2017}.
While \ac{w} can theoretically range from 0 to 1, in practice, \ac{w} is strongly correlated with thermal stress and warm discomfort.
% todo remove consequently below
consequently there is a \ac{w-max} for sustained activity for healthy and acclimatized humans~\cite{ASHRA2017}.

We estimated \ac{w-max} using the following equation from the \mycite{GaggeSET} model:

% todo These equations are presumably from the SET model by Gagge?  Should indicate that.  Also, there are physiological sweat rate control equations that affect wmax, right? Do they need to be described?
...
Few data are available for the Sub-Saharan Africa where approximately 40~\% of the poorest people in the world reside and where climate change may be an acute threat~\cite{PovertyO1:online}.
...
However, it does not contain information about the \ac{rh} which is important for determining whether the use of electrical fans would have been beneficial or not.
