...
% todo SS this is a good addition. Could you also add the key quartiles (2.5; 5; 25; 50; 75; 95, 97.5) for male and female? I am concern that 1.8 may be ok for some groups but not for others.
...
It could be argued that some people during heatwaves may be wearing much fewer clothes than that, hence, a value of \ac{clo} equal to 0.22 clo (i.e., walking shorts, short-sleeve shirt, and sandals) would be more appropriate, however, we wanted to use a more conservative value.
% todo ss for sandals+men underwear, walking shorts and T-shirt I got 0.22 in the comfort tool. I would argue that during a heat wave I may even get rid of t-shirt. Moreover, for example for a female:  sandals,  walking shorts and bra=0.14. I would give at least two examples.
...
The \ac{phs} it is not validated for metabolic rates lower than 1.7 met (100 W/m\textsuperscript{2}), \ac{t-db} higher than 50~$^{\circ}$C and water vapour partial pressure ($p_{a}$) higher than 4~kPa.
% todo SS report this in absolute HR
...
% todo SS I would report only the constant humidity ratio and mention that another method was explore and this was found to be superior. All the details are in the appendix. These two methods are moving the focus of the reader to a secondary aspect for the overall paper.
...
\textit{Constant humidity ratio} -- Firstly, we determined the humidity ratio for each location using the \ac{t-db} corresponding to the hottest 0.4~\% of annual cumulative frequency of occurrence and the mean coincident \ac{t-wb}.
...
We are aware that this assumption has some limitations, however, we deemed to be an acceptable approximation since available alternative may have not lead to better results. One possible solution was to simulate several building archetypes under different climatic conditions.
