...
We used the heat balance model that \mycite{GaggeSET} developed to derive the \ac{set} to determine when the use of elevated air moment (e.g., \ac{v} $>$ 0.2 m/s) would be beneficial to cool people.
...
Moreover, we added it to the \verb|pythermalcomfort| Python package~\cite{Tartarini2020a} and the CBE Thermal Comfort Tool~\cite{Tartarini2020}.
...
Several other equations have been developed to estimate \ac{body-a} and in the \verb|pythermalcomfort| tool we allow users to specify \ac{body-a} as input value.
A review paper reviewed different body surface area formulae and concluded that most of the proposed equations in the literature were in agreement with each other to estimate \ac{body-a} for adults with a healthy weight and standard physique~\cite{Redlarski2016}.
...
The \ac{e} is close to unity (typically 0.95) and \ac{sigma} is a constant.
The value of \ac{t-op} varies as a function of \ac{h-c}, \ac{h-r}, \ac{t-r} and \ac{t-db}, and it is described by:
...
We estimated the \ac{w-max} value and the \ac{m-sweat} using the following equation:
...
% todo add references
These values are higher than those estimated by the \mycite{GaggeSET} model and these values are presented in Figure~\ref{fig:comparison_models}.
...
User can, however, enter this value as input in the \verb|pythermalcomfort| equation.
...
% todo better describe this point, and reference the skin blod flow equation in the appendix
...
This is an approximation and it is arguably overestimating the most extreme condition due to the fact that the likelihood that both conditions would occur at the same time is low.
...
Conditions indoors may, however, be slightly less severe than outdoors since the thermal mass of the building would dump and shift peaks in outdoor temperature but also more severe if there is a significant amount of internal load or solar gains.
...
The database was last updated in August 2020 and we used it to gather information about the number of people who live in the 115 largest cities in the world.
When available we selected the population of the urban agglomerate rather than of the city administrative boundary.
We then combined the population with the weather data to determine: i) how many people were at high risk of experiencing \ac{t-db} higher than 35~$^{\circ}$C\@; and ii) how many people would benefit from the use of electrical fans.
...
A full list of the cities we included in the analysis is provided in the Appendix.
% todo reference appendinx
