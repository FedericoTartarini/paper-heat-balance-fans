...
This percentage decreases to \var{per_location_fans_beneficial_45}~\% if \ac{v} is increased to 4.5~m/s.
This does not imply that on average increasing the \ac{v} from 0.8~m/s to 4.5~m/s has an overall negative effect.
On the other hand, the opposite is true and increasing \ac{v} is beneficial in \var{per_location_fans_beneficial_45}~\% of the locations worldwide.
However, in a few locations characterized by temperature above approximately 44~$^{\circ}$C and \ac{rh} lower than 35~\% increasing \ac{v} from 0.8~m/s to 4.5~m/s would be detrimental.
The same is true for approximately \var{per_location_evaporative_cooling}~\% where electric fans should not be used unless any form of cooling (including evaporative cooling) is used to pre-cool the air to any point in the green region.
...
This number decreases to \var{cities_benefit_fan_45} for \ac{v} up to 4.5~m/s
